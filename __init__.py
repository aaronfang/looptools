...
def get_strokes(self, context):
    ...
    if looptools.gstretch_use_guide == "None":
        return False

    strokes = gstretch_get_strokes(self, context, report=False)
    if strokes:
        return True

    if looptools.gstretch_use_guide == "Annotation":
        self.report({'WARNING'}, "active Annotation strokes not found")
    elif looptools.gstretch_use_guide == "GPencil":
...
class gstretch_fake_stroke():
    def __init__(self, points, source=None):
        ...
        self._gstretch_source = source


# fake stroke point class, used in fake strokes
class gstretch_fake_stroke_point():
    def __init__(self, loc):
        self.co = mathutils.Vector(loc)


# get a stroke point coordinate, compatible with Annotation and Grease Pencil v3
def gstretch_point_co(point):
    if hasattr(point, "co"):
        return mathutils.Vector(point.co)
    if hasattr(point, "position"):
        return mathutils.Vector(point.position)
    return mathutils.Vector()


# get active Annotation data-block, compatible with Blender 4.5
def gstretch_get_annotation_data(context):
    for data in (
        getattr(context, "annotation_data", None),
        getattr(getattr(context, "scene", None), "annotation", None),
    ):
        if data is not None:
            return data

    annotations = getattr(bpy.data, "annotations", None)
    if annotations:
        try:
...
# get active layer for Annotation or Grease Pencil data
def gstretch_get_active_layer(data):
...
# get the frame currently shown on a layer
def gstretch_get_active_frame(layer):
    if layer is None:
        return None

    try:
        current_frame = getattr(layer, "current_frame", None)
...
# get strokes from an AnnotationFrame or GreasePencilFrame
def gstretch_get_frame_strokes(frame):
    if frame is None:
        return None, None
    ...
    try:
        ...
        if drawing is not None:
            return drawing.strokes, drawing
    except Exception:
        pass
...
# copy strokes to the coordinate format expected by Gstretch
def gstretch_prepare_strokes(strokes, matrix=None, drawing=None):
    if not strokes:
        return []
    ...
    for index, stroke in enumerate(strokes):
        ...
        world_points = []
        for point in points:
            co = gstretch_point_co(point)
            raw_points.append(co.copy())
            world_points.append((matrix @ co) if matrix is not None else co.copy())

        if not world_points:
            continue

        if matrix is None and hasattr(points[0], "co"):
            prepared.append(stroke)
        else:
            source = None
            if drawing is not None:
                source = {"drawing": drawing, "index": index, "points": raw_points}
            prepared.append(gstretch_fake_stroke(world_points, source))

    return prepared


# resolve strokes for the selected Gstretch guide mode
def gstretch_resolve_strokes(context):
    looptools =  context.window_manager.looptools

    if looptools.gstretch_use_guide == "Annotation":
        data = gstretch_get_annotation_data(context)
...
# test whether a stroke came from Blender's legacy Annotation/GPencil API
def gstretch_is_legacy_stroke(stroke):
    for type_name in ("AnnotationStroke", "GPencilStroke"):
        stroke_type = getattr(bpy.types, type_name, None)
        if stroke_type is not None and isinstance(stroke, stroke_type):
            return True
    return False


# remove a stroke from a Grease Pencil v3 drawing, if possible
def gstretch_remove_from_drawing(stroke):
    source = getattr(stroke, "_gstretch_source", None)
    if not source:
        return False
    ...
    def matches(candidate):
        ...
        if points is None or len(points) != len(raw_points):
            return False
        for point, raw in zip(points, raw_points):
            if (gstretch_point_co(point) - raw).length > 1e-5:
                return False
        return True

    try:
        index = source.get("index")
        if index is not None and index < len(drawing.strokes) and matches(drawing.strokes[index]):
            drawing.remove_strokes(indices=(index,))
            return True
    except Exception:
        pass

    try:
        for index, candidate in enumerate(drawing.strokes):
...
# remove all strokes from a Grease Pencil v3 drawing
def gstretch_clear_drawing(drawing):
    try:
        indices = tuple(range(len(drawing.strokes)))
        if indices:
...
# remove all frames/strokes from Annotation data
def gstretch_clear_annotation(context):
    data = gstretch_get_annotation_data(context)
    if data is None:
        return False

    removed = False
    for layer in list(getattr(data, "layers", [])):
...
# remove all strokes from a Grease Pencil object
def gstretch_clear_gpencil_object(guide):
    data = getattr(guide, "data", None)
    if data is None:
        return False
    ...
    for layer in list(getattr(data, "layers", [])):
        ...
        for frame in list(frames):
            strokes, drawing = gstretch_get_frame_strokes(frame)
            if drawing is not None:
                removed = gstretch_clear_drawing(drawing) or removed
            elif strokes:
                try:
                    frames.remove(frame)
                    removed = True
...
def gstretch_calculate_verts(loop, stroke, object, bm_mod, method):
    ...
    for i, p in enumerate(stroke.points[1:]):
        stroke_length += (gstretch_point_co(p) -
            gstretch_point_co(stroke.points[i])).length
    projection_tolerance = max(1e-2, stroke_length * 1e-4)
    endpoint_tolerance = min(0.1,
        projection_tolerance / max(stroke_length, 1e-7))

    # return the closest intersection of line with stroke, or None
    def intersect_line_stroke(vec1, vec2, stroke, reference=None):
        if reference is None:
            reference = vec1
        closest = None
        closest_distance = None
        for i, p in enumerate(stroke.points[1:]):
            p_co = gstretch_point_co(p)
            prev_co = gstretch_point_co(stroke.points[i])
            intersections = mathutils.geometry.intersect_line_line(vec1, vec2,
                p_co, prev_co)
            if intersections and \
            (intersections[0] - intersections[1]).length <= projection_tolerance:
                x, dist = mathutils.geometry.intersect_point_line(
                    intersections[1], p_co, prev_co)
                if -endpoint_tolerance <= dist <= 1 + endpoint_tolerance:
                    dist = min(max(dist, 0.0), 1.0)
                    point = p_co.lerp(prev_co, dist)
                    distance = (point - reference).length
    ...
    # fallback: snap to the closest point on the stroke if projection misses
    def closest_point_on_stroke(vec, stroke):
        closest = None
        closest_distance = None
        for i, p in enumerate(stroke.points[1:]):
            p_co = gstretch_point_co(p)
            prev_co = gstretch_point_co(stroke.points[i])
            if (p_co - prev_co).length < 1e-12:
                continue
            point, dist = mathutils.geometry.intersect_point_line(
                vec, p_co, prev_co)
            if dist < 0:
                point = p_co
            elif dist > 1:
                point = prev_co
            distance = (vec - point).length
            if closest_distance is None or distance < closest_distance:
                closest = point
                closest_distance = distance
        if closest is None and stroke.points:
            closest = gstretch_point_co(stroke.points[0])
        return(closest)
    ...
    if method == 'project':
        ...
        def endpoint_from_neighbor_direction(v_index, neighbor_index, current):
            if neighbor_index not in projected:
                return(current)
            current_loc = object.matrix_world @ bm_mod.verts[v_index].co
            neighbor_loc = object.matrix_world @ bm_mod.verts[neighbor_index].co
            neighbor_delta = projected[neighbor_index] - neighbor_loc
            if neighbor_delta.length < 1e-12:
                return(current)
            expected = current_loc + neighbor_delta
            intersection = intersect_line_stroke(current_loc, expected, stroke,
                expected)
            if intersection is not None:
                return(intersection)
            closest = closest_point_on_stroke(expected, stroke)
        ...
        for v_index in loop[0]:
            ...
            if intersection is None:
                ...
                intersection = intersect_line_stroke(
                    current_loc,
                    object.matrix_world @ (v.co + v.normal),
                    stroke,
                    current_loc)
            if intersection is None:
                intersection = closest_point_on_stroke(current_loc, stroke)

            if intersection is not None:
                projected[v_index] = intersection

        if not loop[1] and len(loop[0]) > 2:
            first = loop[0][0]
            second = loop[0][1]
            last = loop[0][-1]
            before_last = loop[0][-2]
            if first in projected:
                projected[first] = endpoint_from_neighbor_direction(first,
                    second, projected[first])
            if last in projected:
                projected[last] = endpoint_from_neighbor_direction(last,
                    before_last, projected[last])

        for v_index in loop[0]:
            if v_index in projected:
                move.append([v_index, matrix_inverse @ projected[v_index]])
...
conversion_distance, conversion_max, conversion_min, conversion_vertices):
    ...
    for stroke in strokes:
        ...
        if conversion == 'distance':
            ...
            prev_co = gstretch_point_co(prev_point)
            stroke_verts[-1][1].append(bm_mod.verts.new(mat_world @ prev_co))
            distance = 0
            limit = conversion_distance
            for point in stroke.points:
                point_co = gstretch_point_co(point)
                new_distance = distance + (point_co - prev_co).length
                iteration = 0
                while new_distance > limit:
                    to_cover = limit - distance + (limit * iteration)
                    new_loc = prev_co + to_cover * \
                        (point_co - prev_co).normalized()
        ...
        # creation of new vertices for other methods
            ...
            for point in stroke.points[:end_point]:
                point_co = gstretch_point_co(point)
                stroke_verts[-1][1].append(bm_mod.verts.new(mat_world @ point_co))
...
def gstretch_erase_stroke(stroke, context):
    if gstretch_remove_from_drawing(stroke):
        return

    if not gstretch_is_legacy_stroke(stroke):
    ...
    erase_stroke = [sp(gstretch_point_co(p), context) for p in stroke.points]
    if erase_stroke:
        erase_stroke[0]['is_start'] = True

    gpencil_ops = getattr(bpy.ops, "gpencil", None)
    if gpencil_ops and hasattr(gpencil_ops, "data_unlink"):
        gpencil_ops.data_unlink()
...
def gstretch_eval_stroke(stroke, distance, stroke_lengths_cache=False):
    ...
    if not stroke_lengths_cache:
        lengths = [0]
        for i, p in enumerate(stroke.points[1:]):
            lengths.append((gstretch_point_co(p) -
                gstretch_point_co(stroke.points[i])).length + lengths[-1])
    ...
    if distance in stroke_lengths:
        loc = gstretch_point_co(stroke.points[stroke_lengths.index(distance)])
    elif distance > stroke_lengths[-1]:
        # should be impossible, but better safe than sorry
        loc = gstretch_point_co(stroke.points[-1])
    else:
        ...
        interval_vector = (gstretch_point_co(stroke.points[stroke_index]) -
            gstretch_point_co(stroke.points[stroke_index - 1]))
        loc = gstretch_point_co(stroke.points[stroke_index - 1]) + distance_relative * interval_vector
...
# get strokes
def gstretch_get_strokes(self, context, report=True):
    strokes = gstretch_resolve_strokes(context)
    if not strokes:
        if report:
            get_strokes(self, context)
        return(None)
...
def gstretch_match_single_verts(bm_mod, strokes, mat_world):
    ...
    endpoints = []
    for stroke in strokes:
        endpoints.append((mat_world @ gstretch_point_co(stroke.points[0]), stroke, 0))
        endpoints.append((mat_world @ gstretch_point_co(stroke.points[-1]), stroke, -1))
...
def gstretch_true_to_safe_strokes(strokes):
    safe_strokes = []
    for stroke in strokes:
        safe_strokes.append([gstretch_point_co(p).copy() for p in stroke.points])
...
class RemoveAnnotation(Operator):
    ...
    def execute(self, context):
        ...
        if not gstretch_clear_annotation(context):
...
class RemoveGPencil(Operator):
    ...
    def execute(self, context):
        ...
        looptools =  context.window_manager.looptools
        if looptools.gstretch_guide is None or \
        not gstretch_clear_gpencil_object(looptools.gstretch_guide):
...
class GStretch(Operator):
    ...
    def execute(self, context):
        ...
        # avoid converting GP strokes to mesh when selected geometry exists but
        # no valid edge loop could be detected
        selected_geometry = any(v.select and not v.hide for v in bm.verts)
        if not loops and selected_geometry:
            self.report({'WARNING'}, "GStretch: selected geometry found, but no valid edge loop detected")
            if derived:
                bm_mod.free()
            terminate()
            return{'CANCELLED'}
